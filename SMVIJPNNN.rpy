...
label SMVIJPNNN_label_1:
    ...
    "Тишину в комнате разбавляет только шум кулера да милое посапывание Леры."
    th "Нам пора."
    me "Лера..."
    ...
    $ set_mode_nvl()
    "Встретились мы с Лерой давно. Это было года три назад. {w}Спустя год нашего после нашего знакомства, она поступила в институт, где я учился на втором курсе."
    "Когда мы начали общаться между нами не было стены робости и стеснения."
    ...
    show blink
    "Глаза начали слипаться и я медленно начал проваливаться в небытие."
    ...
    me "Всего 9, считая персонал. По крайней мере столько раскрыто в игре. Здесь может быть больше."
    ...
    "У неё это всегда получается хорошо, и даже в такой напряжной, незнакомой ситуации я просто не смог сдержать смех."
    me "Да, хорошо."
    ...
    me "Но несмотря на это простынь под тобой проминается."
    ...
    extend "Славя."
    sl "А... {w}А как это?"
    "Лера всё стояла и держала мой галстук. Со стороны это выглядело, как будто концы его левитируют."
    ...
    th "Ну наконец-то..."
    "Лера схватила концы галстука и начала поднимать и опускать их."
    ...
    sl "Вау, это замечательно. Только ты галстук не завязал до конца. "
    ...
    menu:
        "Позволить Славе завязать":
            ...
            sl "Хорошо, спасибо."
            hide sl with dspr 
            "Славя ушла, оставив нас с Лерой в напряженной обстановке."
    ...
    "Всю дорогу до поляны мы молчали. Это была вынужденная мера, потому что сейчас середина дня и большинство пионеров находятся на улице."
    ...
    dv "О, какие люди. Я тебя тут раньше не видела. Это ты тот новенький, про которого говорила Уля?"
    ...
    me "М?"
    "Я посмотрел на свою руку. {w}Она вся была в чернилах."
    ...
    "Подножка не заставила себя долго ждать и Алиса упала на землю." with vpunch
    ...
    me "Да Лера, тут необычайно красиво."
    ...
    "Не успел я договорить, как меня прервал горн призывающий к ужину."
    ...
    me "Мне кажется тебе лучше потом поесть, не прилюдно."
    Ler_SM "Почему?"
    ...
    menu:
        "Уступить место Славе":
            ...
            Ler_SM "Тогда я постою."
            th "Может стоило сделать по другому? Ладно, время вспять не повернуть."
            sl "Как тебе наш лагерь, Семен?"
            me "Я пока что не знаю, первый день все же."
            "Сказал я улыбнувшись."
            sl "Ну ничего, тебе у нас понравится."
            "Я доел свой ужин, хотел уже вставать, но тут Лера с обидой сказала."
            
            
        "Извини, я кое-кого жду":
            $ ES_Lera_comfort_points_SMVIJ += 1
            show sl angry pioneer at center  with dspr
            th "Возможно это ее обидело, но я не могу согнать Леру с места."
            sl "Ладно, пойду еще место поищу."
            hide sl with dspr
            Ler_SM "Да, кстати приятного аппетита."
            me "Спасибо."
            window hide
            $ renpy.pause (2)
            "Я доел свой ужин, хотел уже вставать, но тут Лера с улыбкой сказала."
            
        
        
    Ler_SM "Доел? {w}Пойдем."   
    
    hide sl with dissolve
    "Я вспомнил, что Лера так и не поела."
    "Я решил взять ей немного еды."
    "Я сложил бутерброды в пакетик, но тут появилась наша вожатая."
    show mt normal pioneer at center with dissolve
    mt "Семен, кому еду складываешь?"
    "Черт, попался, так думай Семен, для кого еда?"
    me "Эм... Кристине, она ведь не поела, решила на пляже посидеть."
    mt "Ну ладно, передай ей, чтобы зашла ко мне."
    me "Хорошо, передам."
    th "Фух, пронесло..."
    "Я поскорее удалился из столовой."
    hide mt with dissolve
    play ambience ambience_camp_center_evening fadeout 2 fadein 1
    scene ext_dining_hall_away_sunset with dissolve
    "Мы вышли из столовой. Закат был невероятно красив."
    "Лицо Леры переливалось разными красками, она была очень красива в этот момент."
    "Я не выдержал и прервал наше молчание."
    me "Слушай, а ты прекрасно выглядишь в закате." 
    
    if d1_Lera_diner_ploho: 
        "Лера посмотрела на меня пустыми глазами." 
        Ler_SM "Угу." 
    else: 
        "Лера посмотрела на меня счастливыми глазами." 
        Ler_SM "А с чего бы эти комплименты?" 
        Ler_SM "Но спасибо, мне приятно."
    
    me "Пойдем на пляж, передадим послание Кристине?"
    
    if d1_Lera_diner_ploho:
        Ler_SM "Извини, я домой пойду, там тебя подожду."
        me "Как хочешь."
        "Я передал ей бутерброды и направился на пляж."
        scene ext_square_sunset with dissolve 
        $ renpy.pause (2)
        scene ext_path_sunset with dissolve
        $ renpy.pause (2)
        ...
        $ set_mode_nvl()
        "Сегодняшний случай с Лерой не давал мне покоя, плохо я с ней обошелся, но что я могу сделать."
        "Я всегда мечтал попасть в Совенок, провести тут время с этими пионерками."
        "И вот моя мечта сбылась, я в Совенке. Сегодня я обидел Леру, позже мне предстоит разговор с ней."
        "Странно, что в игре и в реальности, этот лагерь отличается. Я всегда представлял себе Совенок совсем другим."
        "Пионеры не очень схожи с тем что я видел в игре."
        "Славя совершенно не такая, как я себе представлял. Какая-то она другая, не знаю почему, но не такая."
        "Алиса и Ульяна, тоже какие-то не такие."
        "Надо будет с этим разобраться."
        "Еще одна странность заключается в том, что Леру видит одна лишь Кристина. Почему?"
        ...
        "Женя, как всегда, вредничать будет, надо будет ее разбудить специально, чтобы позлить."
        "И Мику в своей пикантной позе."
        stop music fadeout 5
        window hide 
        $ set_mode_adv()
        "Так я просидел минут 20."
        th "Пора идти, что-то я засиделся."
        "Я отправился к домику."
    else:
        Ler_SM "Конечно, пошли."
        "Сказала она улыбнувшись."
        "Я передал ей бутерброды, чтобы она поела по пути и мы держась за руки направились на пляж."
        scene ext_square_sunset with dissolve 
        $ renpy.pause (2)
        scene ext_path_sunset with dissolve
        $ renpy.pause (2)
        play ambience ambience_boat_station_day fadeout 2 fadein 1
        scene ext_beach_sunset with dissolve
        "Мы пришли на пляж и увидели Кристину. {w}Она плакала."
        ...
        Vlr_SM "Ну ладно, я тогда пойду к вожатой."
        ...
        $ set_mode_nvl()
        "Сегодняшний я не хорошо поступил с девочками, плохо я с ними обошелся, но что я могу сделать."
        "Я всегда мечтал попасть в Совенок, провести тут время с Лерой."
        "И вот моя мечта сбылась, мы в Совенке."
        "Странно, что в игре и в реальности, этот лагерь отличается. Я всегда представлял себе Совенок совсем другим."
        "Пионеры не очень схожи с тем что я видел в игре."
        "Славя совершенно не такая, как я себе представлял. Какая-то она другая, не знаю почему, но не такая."
        "Алиса и Ульяна, тоже какие-то не такие."
        ...
        "Даже возвращаться назад особо не хочется, но на седьмой день меня это и ждет."
        "Это меня очень печалило."
        "Завтра мы должны встретиться с Мику, Виолой, Женей."
        "Виола начнет меня соблазнять, я уже знаю что делать."
        "Женя как всегда вредничать будет, надо будет ее разбудить специально, чтобы позлить."
        "И Мику в своей пикантной позе, надо будет не пялиться, чтобы Лера не приревновала."
        $ set_mode_adv()
        "Так я просидел минут 20."
        th "Пора идти, что-то я засиделся."
        stop music fadeout 2
    ...
    play ambience ambience_int_cabin_evening fadeout 2 fadein 1
    $ renpy.pause (2)
    scene int_house_of_mt_sunset with dissolve
    
    
    
    "Вернувшись домой меня одолела легкая усталость."
    "Вроде я уже должен был предугадать каждый шаг девушек в лагере, но некоторые обстоятельства, видимо, повлияли на это."
    "Один из них, как минимум, это то, что я тут с Лерой."
    if d1_Lera_diner_ploho:
        th "А где Лера? {w}Она же говорила, что будет ждать меня здесь."
        me "Ну, может быть, прогуляться решила."
        "Сказал я себе под нос."
        "Присев на кровать, я стал пристально рассматривать свои ноги."
        "Мысли были совершенно разные, но ни одна из них не задерживалась надолго."
        th "Может пойти Леру поискать?"
        th "Да и где мне её искать?"
        ...
        menu:
            "Пойти искать Леру":
                ...
                play ambience ambience_camp_center_evening fadeout 2 fadein 1
                scene ext_house_of_mt_sunset with dissolve
                "Свет вечернего солнца снова встретил меня."
                "Мои глаза мельком пробежали по округе."
                scene ext_houses_sunset with dissolve
                "Мой взгляд зацепился за едва знакомую фигуру вдалеке."
                th "Кристина?"
                "Это вполне могла быть она. {w}Её фигура очень схожа с фигурой Леры. Да и лицо слегка похоже, а вот прическа другая, Лера бы такую не стала носить. Она больше по коротким прическам."
                me "Кристина!" with vpunch
                $ Krst_comfort_SMVIJPNNN += 1
                "Повысил голос я с целью, чтобы мои слова достигли её. Она явно была в своих мыслях, так что могла меня и не услышать."
                ...
                Ler_SM "Пошли домой?"
                "Сказала мне Лера, хватаясь за мою руку, словно маленький ребенок."
                me "Ладно, пойдем."
                me "Пока, Кристина."
                "Но когда я обернулся, Кристины уже и след простыл."
                th "Что это с ней? {w}Что-то у неё с настроением под конец дня странное происходит."
                th "Может быть у неё эти..."
                Ler_SM "Ну пошли? Чего стоишь?"
                "Не успел я додумать, как Лера меня зовёт."
                me "А, да. Пошли."
                "Взявшись за руки мы направились к домику."
                scene ext_house_of_mt_sunset with dissolve
                $ renpy.pause (2)
                play ambience ambience_int_cabin_evening fadeout 2 fadein 1
                scene int_house_of_mt_sunset with dissolve
                "Зайдя внутрь, мы сели на кровать и синхронно выдохнули."
                ...
                "Вот так мы и провели остаток вечера, обмениваясь эмоциями за сегодняшний, не самый легкий морально, день."
                me "Спать пойдем?"
                Ler_SM "А где я спать буду?"
                me "А ты как думаешь? {w}Домик мы тебе не сможем выделить, не в наших силах. Будешь спать со мной."
                "Лера бросилась в краску, но ответила быстро."
                Ler_SM "Ну да, хорошо."
                me "А чего ты стесняешься? {w}Уже как несколько лет вместе."
                Ler_SM "Ну так я буду спать с тобой прямо перед Ольгой или как там её."
                me "Так она тебя всё равно не видит."
                Ler_SM "Легче от этого не становится, всё равно давит на меня её присутствие."
                me "Ну тогда ложись к стене, чтобы не видеть её."
                "Лера посмотрела на место у стенки и высказала своё согласие."
                Ler_SM "Да, я не против. {w}Так и поступим."
                play sound sfx_knock_door6_closed
                "В дверь постучали"
                show mt normal pioneer at center with dissolve
                extend " и вошла вожатая."
                mt "Семён, а ты что не спишь? {w}Завтра начинается твой полноценный день в лагере."
                me "Да знаю я, вот уже собирался ложиться."
                show mt smile pioneer at center
                ...
                Ler_SM "Ну не буду же я переодеваться сидя, дурачок."
                th "А верно..."
                me "Да, хорошо."
                "Лера пошла к выключателю."
                scene int_house_of_mt_night2 with dissolve
                "Выключив свет она вернулась на место."
                "Лера начала снимать с себя одежду. {w}Её стройная, любимая мною фигура еще изящнее выглядела в свете луны."
                "На пол упали джинсы. {w}Утепленные, кстати."
                th "Как она весь день на такой жаре в них проходила? {w}Надо будет найти ей что-то более подходящее."
                Ler_SM "Ну всё, я готова ко сну."
                "Лера престала передо мной в тонкой маечке и ничего более кроме нижнего белья."
                "Луна как будто дразнила меня, отражаясь на всех изгибах её фигуры."
                Ler_SM "Ну что, я не знаю как ты, а я пойду спать. {w}Подвинься."
                "Лера аккуратно пихнула меня в бок и залезла под одеяло."
                "Я поднял свою челюсть и начал тоже переодеваться."
                Ler_SM "Кстати, а где наши вещи, которые мы с собой брали? {w}Я просто не выдержу неделю, как ты сказал, в джинсах по такой жаре."
                me "Я завтра что-нибудь найду, положись на меня."
                Ler_SM "Хорошо. {w}Спокойной ночи."
                me "Спокойной ночи..."
            ...
            "Остаться в домике":
                ...
                "Я прилег на кровать, закинул нога на ногу и стал смотреть в потолок."
                "Пытаясь разгадать тайну моего попадания сюда, я чуть не сошел с ума."
                "Столько странностей, что можно и реально поехать."
                "С сюжетом игры осталось мало общего, так что будет дальше?"
                play sound sfx_knock_door6_closed
                "В дверь постучался кто-то."
                show mt normal pioneer at center with dissolve
                mt "Семён, что лежишь?"
                me "Да так, думаю о своём."
                show mt smile pioneer at center
                mt "Так как тебе в лагере?"
                me "Пока не оценил всё, что можно. {w}Я же здесь всего половину дня."
                mt "Ну да. {w}Ничего, завтра лучше лагерь узнаешь. Поверь, тебе тут понравится."
                me "Я и не сомневаюсь."
                "Сказал я улыбаясь."
                "Тут дверь отворилась и кое-кто показался за дверью."
                th "А вот и она."
                show mt surprise pioneer at center
                mt "Что?"
                show mt normal pioneer at center
                extend " Может сквозняк."
                "Лера быстро прошмыгнула в дом и присела ко мне."
                "В это время вожатая взялась за ручку двери и посмотрела в мою сторону."
                mt "Ладно, Семён, я пойду посмотрю кто еще гуляет, время уже к отбою близится, все уже должны быть по домам. {w}Поставь пакет, пожалуйста, на стол."
                me "Да, хорошо."
                "Я встал и забрал пакет, протянутый мне."
                mt "Всё, я пошла."
                hide mt with dissolve
                "Дверь захлопнулась за Ольгой Дмитриевной."
                Ler_SM "Кое-как проскочила."
        ...
        play sound sfx_knock_door6_closed
        "Не успел я и моргнуть, как кто-то постучал в дверь."
        th "Ну вот и она."
        show mt normal pioneer at center with dissolve
        extend " Ну почти угадал."
        mt "Семён, что сидишь?"
        me "Да так, думаю о своём."
        show mt smile pioneer at center
        mt "Так как тебе в лагере?"
        ...
        hide mt with dissolve
        "Дверь захлопнулась за Ольгой Дмитриевной."
        Ler_SM "Фух, успела. Чуть не прихлопнула, как комара."
        me "Ага, я бы в твоём положении был аккуратнее."
        Ler_SM "Ты так говоришь, как будто я беременна."
